...
public class ghost : MonoBehaviour {
	...
	void Update () {
        ...
        {
            if(transform.position.x - player.transform.position.x <= appearDifferX)
            {
    ...
    {
        if(collision.tag == "Player" && hasAppeared && !hasDisappeared)
        {
            collision.gameObject.GetComponent<Player>().GetHurt();
        }
    }
